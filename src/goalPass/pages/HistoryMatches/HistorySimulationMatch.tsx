...
export const HistorySimulationMatch = () => {
  const { id } = useParams();
  const { getSimulationMatchQuery } = useQuerySimulation(id!);
  const navigate = useNavigate();

  if (getSimulationMatchQuery.isLoading) {
    return (
  ...
  return (
    <div className="overflow-y-auto p-6">
      ...
      >
        ...
        <p className="font-extrabold">Volver a historial</p>
      </button>
      <motion.div
        ...
        className="mt-4 xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 grid gap-4 flex-wrap w-full max-w-[1123px] mx-auto"
      >
        {/* Marcador */}
              ...
              <div className="mt-2 flex flex-col gap-4">
                <div className="w-full bg-gray-2-custom h-[52px] rounded-[15px] py-3 px-8 flex justify-between items-center">
                  ...
                  <p className="font-bold">
                    ...
                    {
                      getSimulationMatchQuery.data?.data?.simulation?.stats
                        ?.local?.goals_by_half?.first_half
                    }
                  </p>
                </div>
                <div className="w-full bg-gray-2-custom h-[52px] rounded-[15px] py-3 px-8 flex justify-between items-center">
            ...
            <div className=" h-auto">
              ...
              <div className="mt-2 flex flex-col gap-4">
                ...
                <div className="w-full bg-gray-2-custom h-[52px] rounded-[15px] py-3 px-8 flex justify-between items-center">
                  ...
                  {
                    getSimulationMatchQuery.data?.data?.simulation?.stats
                      ?.visitor?.goals_by_half?.second_half
                  }
                </div>
              </div>
            </div>
        ...
        <div className="w-full max-h-[550px] rounded-[15px] bg-black-2-custom col-span-full p-8 flex flex-col">
          ...
          <div className="mt-6 flex-1 overflow-y-auto flex flex-col gap-4 pr-4">
            {getSimulationMatchQuery.data?.data?.timeline?.events?.map(
              (event) => (
